...
> 方案一：利用数据库的 FOR UPDATE SKIP LOCKED 特性
>> 试图锁定行，如果该行已经被别人锁定了，就直接跳过，去找下一行未被锁定的数据，变相实现数据库层面的“并发队列”

> 方案二：号段模式（Leaf-segment模式）预加载，LinkedBlockingQueue/Redis
1. 预分配到队列，数据库标记已分配
2. 队列库存低时，异步回补，数据库标记已分配
3. 队列消费，更新数据库标记
...
* [https://github.com/Totoro-jam/battle-tested-patterns](https://github.com/Totoro-jam/battle-tested-patterns)
    * [https://totoro-jam.github.io/battle-tested-patterns/zh/patterns](https://totoro-jam.github.io/battle-tested-patterns/zh/patterns)
...
| 模型分类 / 名称 | 核心机制 & 关键词 | 适用场景 & 核心特点 | 代表技术与框架 |
| :--- | :--- | :--- | :--- |
| **共享内存 / 锁**<br>Shared Memory & Locks | `Mutex`、`Monitor`、`CAS`、`Lock-Free`、条件变量 | **通用多线程、底层开发**<br>特点：契合硬件、性能极高，但极易产生死锁和数据竞争。 | Java、C++、C#、POSIX Threads |
| **通信顺序进程**<br>CSP (Communicating Sequential Processes) | `Channel` (匿名通道)、同步/缓冲通信、`select` | **微服务并发、数据流水线**<br>哲学：“不要通过共享内存来通信，而应通过通信来共享内存”。发送接收高度解耦。 | Go (Goroutines)、Clojure (`core.async`)、Occam |
| **Actor 模型**<br>Actor Model / 消息传递 | `Actor`、`Mailbox`、纯异步消息、**状态隔离** (无共享) | **分布式系统、高可用与容错**<br>哲学：“万物皆 Actor”与“任其崩溃 (Let it crash)”。天然无锁，易横向扩展。 | Erlang / Elixir、Akka (JVM)、Microsoft Orleans |
| **软件事务内存**<br>STM (Software Transactional Memory) | `atomic`、`retry`、**乐观锁 (OCC)**、事务回滚 | **复杂状态共享、多变量原子更新**<br>特点：将数据库事务概念引入内存，避免死锁，极具可组合性（Composable）。 | Haskell STM、Clojure (`ref`)、Scala ZIO |
| **数据流 / 响应式**<br>Dataflow / Reactive | `DAG` (有向无环图)、数据依赖、事件触发、`Observable` | **流式计算、机器学习图计算、UI响应**<br>特点：计算由数据的可用性驱动，而非控制流驱动。 | Flink、TensorFlow、RxJava、LabVIEW |
| **异步 / 事件驱动**<br>Async & Event-Driven | `EventLoop`、`async/await`、`epoll/kqueue`、回调 | **高并发 I/O、海量网络连接 (C10K/C10M)**<br>特点：单线程或少线程承载海量并发，适合 I/O 密集型而非 CPU 密集型。 | Node.js、Python (`asyncio`)、Netty、Redis |
| **数据并行**<br>Data Parallelism | `SIMD`、`SPMD`、向量化、`MapReduce` | **数值计算、图形渲染、大数据分析**<br>特点：将相同的操作同时应用于海量数据元素。 | CUDA / OpenCL (GPU)、Hadoop / Spark |
| **工作窃取**<br>Fork-Join / Work-Stealing | `fork()`、`join()`、双端队列 (Deque) | **分治算法、递归并行计算**<br>特点：空闲线程主动从繁忙线程的队列尾部“窃取”任务，极致利用多核 CPU。 | Java `ForkJoinPool`、Rust (`Rayon`) |
| **函数式 / 不可变**<br>Functional Concurrency | `Immutable`、无副作用 (No Side-effects)、纯函数 | **高可靠并发、状态规约**<br>特点：消除共享可变状态，从根本上杜绝竞态条件，天然线程安全。 | Haskell、Clojure、Scala |
| **行为导向并发**<br>BOC (Behavior-Oriented Concurrency) | `when` 语句、隔离区域 (Regions)、资源所有权 | **无锁并发探索、安全系统级编程**<br>特点：基于所有权和隔离区的细粒度并发控制（较前沿的实验性模型）。 | Microsoft Verona、bocpy |
| *(理论数学补充)*<br>**进程代数 / 进程演算 (Process Calculus / Process Algebra) / Petri网(Petri Nets)** | `π-演算(Pi-calculus)`、`库所/变迁`、`Token 流转` | **形式化验证、协议死锁检测**<br>特点：用于在代码编写前，从数学上证明并发拓扑逻辑的正确性。 | TLA+、BPMN 工作流引擎底座 |

> Rust：“无畏并发”（Fearless Concurrency），基于所有权系统（Ownership）的多范式安全并发，Send：类型可以安全地在线程间转移所有权，Sync：类型可以安全地被多个线程共享引用
