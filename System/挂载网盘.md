...
--vfs-read-chunk-size-limit 1G > /dev/null 2>&1 &
```
...
> 在`vi /etc/rc.local`文件中按<kbd>Shift</kbd> + <kbd>g</kbd>（就是大写的G）跳转到末尾添加挂载命令
...
tmux new -s gdrive -d "/usr/bin/rclone mount onedrive:/ /onedrive \
--copy-links --no-gzip-encoding --no-check-certificate --allow-other \
--allow-non-empty --umask 000"
```
