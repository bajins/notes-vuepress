...
1.  在 **文件 A** 中，按 `Ctrl + ~` （波浪号，通常在 `Esc` 键下面）进入**显示公式模式**。
2.  选中你要复制的所有单元格，`Ctrl + C`。
3.  打开 Windows 自带的 **记事本 (Notepad)**，`Ctrl + V` 粘贴。
4.  在记事本里 `Ctrl + A` 全选，`Ctrl + C` 复制。
5.  回到 **文件 B**，`Ctrl + V` 粘贴。
6.  在文件 A 按 `Ctrl + ~` 恢复正常显示模式。
...
1.  点击 Excel 顶部菜单栏的 **“公式” (Formulas)** 选项卡。
2.  找到最右侧的 **“计算选项” (Calculation Options)**。
3.  检查是否勾选了 **“自动” (Automatic)**。
    *   如果是“手动”，请改为“自动”。
    *   或者，你可以按一下键盘上的 **`F9`** 键。如果按了之后数值变了，说明就是这个问题。
...
- 输入 `2610`/`25A1` 然后按 `Alt + X` → 得到 ☐
- 输入 `2611` 然后按 `Alt + X` → 得到 ☑
- 输入 `2612` 然后按 `Alt + X` → 得到 ☒

excel：

- `=UNICHAR(9744)`/`=UNICHAR(HEX2DEC("2610"))` → ☐
- `=UNICHAR(9745)`/`=UNICHAR(HEX2DEC("2611"))` → ☑
- `=UNICHAR(9746)`/`=UNICHAR(HEX2DEC("2612"))` → ☒
